...
internal static class ItemTooltipPatches
{
    ...
    internal static string ApplyCraftedByDisplayToTooltipText(string text, ItemDrop.ItemData item)
    {
        if (string.IsNullOrEmpty(text) || item?.m_customData == null)
            return text;
        if (!item.m_customData.TryGetValue(DrakeRenameit.DrakeCraftedByDisplay, out var disp) || string.IsNullOrEmpty(disp))
            return text;
        ...
        disp = TooltipRichText.EnsureColorTagsClosedForTooltip(disp);
        return ReplaceCraftedBySegment(text, oldName, disp);
    }

    /// <summary>Match vanilla <c>\n$item_crafter: name</c>, localized "Crafted by: …", and common &lt;color&gt; wrapping around the name.</summary>
    internal static string ReplaceCraftedBySegment(string text, string oldName, string newDisplay)
    {
